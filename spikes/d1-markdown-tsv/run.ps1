...
$evidenceRoot = Join-Path $repoRoot 'evidence\spikes\d1-markdown-tsv'
...
if ($LASTEXITCODE -ne 0) { throw "library build failed with exit=$LASTEXITCODE" }
...
if ($LASTEXITCODE -ne 0) { throw "spike compilation failed with exit=$LASTEXITCODE" }
...
if ($Mode -eq 'red' -and $exitCode -eq 0) { throw 'RED must fail' }
if ($Mode -eq 'green' -and $exitCode -ne 0) { throw "GREEN failed with exit=$exitCode" }
exit 0
