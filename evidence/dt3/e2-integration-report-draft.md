...
Windows runtime проходит абсолютный service path от доверенного volume root к leaf: каждый уже существующий компонент открывается no-follow, проверяется по handle и удерживается до перехода к следующему; отсутствующие компоненты создаются и немедленно закрепляются. Поэтому подмена существующего промежуточного parent junction-ом либо запрещается ранее закреплённой цепочкой, либо обнаруживается terminal no-follow open до descendant-записи. `runs` закрепляется тем же способом до открытия SQLite/Tantivy. Handles service/runs и каждого exact run-child удерживаются без `FILE_SHARE_DELETE`; marker writes и cleanup выполняются при закреплённом child handle, а пустой каталог помечается на удаление через тот же handle. Детерминированный barrier перед открытием существующего parent и дополнительные coordinated races подтверждают неизменный внешний sentinel и отсутствие внешних DB/index.
