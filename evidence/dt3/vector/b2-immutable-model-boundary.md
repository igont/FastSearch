...
- После no-follow open размер и regular-file type перепроверяются на acquired handle; чтение использует exact-size `read_exact` и однобайтный EOF probe вместо неограниченного `read_to_end`. Disposable grown-file regression (`+1,048,576 B`) даёт bounded rejection: `1 passed`, `2.71 s`.
