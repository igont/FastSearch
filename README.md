...
При первом открытии области с источниками FastSearch автоматически загружает только выбранную модель и выполняет readiness probe. Startup flow не запускает индексацию. Активная модель сохраняется в `.fastsearch/workspace.toml`; `/model` показывает responsive-таблицу каталога, после которой модель можно выбрать следующим вводом одного номера (`1`–`5`) либо сразу командой `/model <номер|slug>`. `/model info <номер|slug>` открывает источник и технические сведения, а явная форма `/model set <номер|slug>` также меняет выбор и требует последующего `/index update` или `/index rebuild`. Машинное назначение устройства задаётся `/model device <номер|slug> [cpu|gpu]`: без последнего аргумента команда переключает CPU/GPU, с аргументом назначает явно. Настройки хранятся в `%LOCALAPPDATA%\FastSearch\device-preferences.toml` и применяются к индексированию и поиску после переоткрытия runtime.
...
Команда `/sources` показывает оба contour, `/sources discover` повторяет automatic discovery, а `/sources set` позволяет задать несколько roots через `;` или отключить contour значением `-`. Поисковая выдача показывает по пять карточек на странице: номер, относительное совпадение в процентах, одну белую строку `Файл` с полным путём и выделенную триггерную фразу длиной до 165 символов без внутренних ID, retrieval channel и raw score. Процент нормализован относительно первого результата текущей выдачи (`100%`) и не является вероятностью или абсолютной оценкой качества. Выдача хранится до следующего поиска: `/open <номер>` открывает результат по стабильному абсолютному номеру, `/related <номер>` показывает связи, `/next`/`/prev` листают страницы, `/page <номер>` переходит к странице, а `/repeat` повторяет текущую выдачу. `/workspace` возвращает к выбору области. `/index rebuild` всегда показывает preview, заранее объясняет ответы `ДА`/`НЕТ` и требует подтверждения. После подтверждения update/rebuild показывает измеримый progress по фактическим этапам: чтение источников, согласование корпуса, полнотекстовый индекс и, если доступен, векторный индекс. Обычный search не скрывает full rebuild.
