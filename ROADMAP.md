...
Измерения подтвердили, что четыре модельные роли помещаются в принятый ресурсный контур, Qwen проходит независимый эталон, а реальный MCP-клиент получает общий ответ. Ближайшая граница B - полная модельная готовность; она ещё не материализована. Производственный координатор, поиск и фильтры входят в отдельную следующую границу C.
...
| B. Модельная готовность | Чистый кэш публикует четыре проверенные модельные роли | принятое основание A и сквозные замеры A5 | ближайшая, не материализована |
...
Для B уже приняты состав ролей, неизменяемые редакции, проверка размеров и SHA-256, продолжение загрузки, блокировка установки, атомарная публикация, вычислительные пробы и маркеры готовности. Приёмка охватывает чистый кэш, повреждённый артефакт, повторное открытие без загрузки и блокировку обычного поиска при неготовности любой роли. Исполнимый план B можно составить без повторного выбора основной архитектуры.
