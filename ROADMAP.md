...
- После принятого основания DT3 в `main` слиты переработка среды исполнения, терминального интерфейса и рабочие области. Исторической редакцией планирования этого слоя была `5416e083a6a0deef936c40f6f64664c5afb41fc5`; прежняя `23ed8773f9830bf6762f058255b17cbb1fe7ad46` не является её предком и не используется как исполнительное основание. Текущая среда исполнения автоматически восстанавливает закреплённый кэш моделей и проверяет неизменяемый манифест без ручного пути; подготовка моделей не запускает индексирование или поиск.
- Основание дерева 4 принято по `GA-FOUNDATION` в редакции `1648708923af48b3f546f758b21d16767084e0eb`. Семантический план `PV-1` исполнен в совместимом корпусе `FASTSEARCH-DT4-FOUNDATION-22-08-2026-PV2`: закрыты A1-A5, проверены Qwen, измеренные пределы, публичный DTO и тонкий реальный MCP-маршрут. Граница B полной модельной готовности выполнена: выпускной исполняемый файл готовит четыре роли из локально проверенных исходных артефактов, публикует согласованное поколение и восстанавливает повреждённую роль без частичного `READY`. Производственный координатор, поиск и фильтры образуют ближайшую границу C; паритет представлений и выпуск остаются последующими границами D и E.
- Исторический [DT3→DT4 handoff](evidence/dt3-dt4-handoff.md) сохраняет принятый DT3 product baseline, но не определяет текущий протокол, модельное состояние или исполнительную редакцию. Для DT4 его заменяют TDR-FS-2.4, TDR-FS-2.6, TDR-FS-2.7 и исполненный план основания.
...
- Основание общего поиска реализовано: Arctic, E5 Large и Nomic возвращают до пяти кандидатов каждая; пул имеет ёмкость 15 позиций, а фактическое количество составляет от 0 до 15. После удаления повторов Qwen3-Reranker-0.6B строит общий порядок, а публичный DTO и тонкий MCP-ответ содержат не более шести результатов. Контур подготовки моделей закрыт границей B; производственный поиск с фильтрами, полный паритет CLI, JSON и MCP и выпуск остаются границами C-E.
...
- `G-EXECUTION-BASE` закрыт на принятой исполнительной редакции A1 `1da4fd01ed22bab6e673ff219e5169714242a89b`. Редакция планирования `aa7a7d45033098b1690691c47b4dadbd5f7b084e` отделена от исполнительных редакций; каждый последующий контрольный рубеж повторён на точной редакции своей стадии, а ветвь A принята без активных замечаний.
...
| `AgentSurface` и `Capability::AgentSurface` | Реальный MCP-адаптер и публичный DTO приняты; состояние транспорта ещё не входит в общий отчёт возможностей | Единая производственная композиция для консоли, JSON и MCP с наблюдаемым состоянием транспорта | 4 |
...
- исполненный план основания DT4 и принятая исполнительная редакция `1da4fd01ed22bab6e673ff219e5169714242a89b`;
...
- параллельно получает до пяти кандидатов от каждой из трёх embedding-моделей, образуя пул ёмкостью 15 позиций, удаляет повторы по канонической записи и передаёт общий набор Qwen reranker;
...
- чистая подготовка получает три embedding-модели и Qwen3-Reranker-0.6B, выполняет проверки ролей и повторно открывает готовый набор без сетевой загрузки;
...
### Принятое основание и ближайшая граница

План `PV-1` закрыл основание DT4 по `GA-FOUNDATION`. Закреплённые зависимости, CPU-адаптер Qwen, публичный DTO, отображение ошибок и `TS-DT4-01` прошли приёмку. Реальный MCP `search` проходит через три актуальные проекции и Qwen и возвращает до шести результатов на ограниченном корпусе.

Граница B подтвердила полный набор из четырёх ролей на целевой Windows-машине. Выпускная команда заново проверяет размеры, SHA-256 и вычислительные контракты, согласованно публикует одно поколение и повторно открывает его без сетевой загрузки. Производственный координатор, поиск и фильтры входят в ближайшую границу C и теперь опираются на принятое модельное основание.

| Граница | Наблюдаемый результат | Основание | Состояние |
|---|---|---|---|
| A. Основание | Реальный MCP-вызов проходит через три проекции и Qwen | `GA-FOUNDATION` | принято |
| B. Модельная готовность | `fastsearch models prepare` из чистого кэша публикует четыре проверенные модельные роли, а `fastsearch models status --json` показывает согласованный снимок | принятое основание A и `G-TS-DT4-02` | принято |
| C. Поиск и фильтры | Производственный поиск обрабатывает метаданные, фильтры и отказы | PASS границы B | ближайшая граница; готова к материализации |
| D. Публичные представления | Консоль, JSON и MCP равны по семантике | PASS границы C | ожидает C |
| E. Выпуск | Windows-сборка проходит чистую установку и проверку MCP | PASS границы D | ожидает D |

Принятая граница B закрывает готовность четырёх модельных ролей, но не объявляет весь поиск готовым. Составное состояние `SEARCH_READY` дополнительно требует актуальных источников и трёх векторных проекций; этот результат относится к границе C.

Для B уже приняты состав ролей, неизменяемые редакции, проверка размеров и SHA-256, продолжение загрузки, блокировка установки, атомарная публикация, вычислительные пробы и маркеры готовности. Команды `fastsearch models prepare` и `fastsearch models status --json` работают до выбора рабочей области и не строят проекции. Приёмка охватывает чистый кэш, повреждение каждой роли, повторное открытие без сетевой загрузки и согласованность снимка при двух процессах.
...
Фактический переход DT2→DT3 зафиксирован в `evidence/dt2-dt3-handoff.md`. Исторический переход DT3→DT4 находится в `evidence/dt3-dt4-handoff.md`: на момент передачи он отделял принятое основание поиска и среды исполнения от ещё не выбранных протокола и контура выпуска. Текущее состояние DT4 определяется принятыми TDR и исполненным планом основания.
