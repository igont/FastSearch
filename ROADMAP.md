...
Измерения подтвердили, что четыре модельные роли помещаются в принятый ресурсный контур, Qwen проходит независимый эталон, а реальный MCP-клиент получает общий ответ. Для ближайшей границы B материализован отдельный план полной модельной готовности; кодовое исполнение откроется только после принятия плана. Производственный координатор, поиск и фильтры входят в отдельную следующую границу C.
...
| B. Модельная готовность | `fastsearch models prepare` из чистого кэша публикует четыре проверенные модельные роли, а `fastsearch models status --json` показывает согласованный снимок | принятое основание A и сквозные замеры A5 | план материализован; ожидает принятия до исполнения |
| C. Поиск и фильтры | Производственный поиск обрабатывает метаданные, фильтры и отказы | PASS границы B | ожидает B |
