...
- Основание дерева 4 принято по `GA-FOUNDATION` в редакции `1648708923af48b3f546f758b21d16767084e0eb`. Семантический план `PV-1` исполнен в совместимом корпусе `FASTSEARCH-DT4-FOUNDATION-22-08-2026-PV2`: закрыты A1-A5, проверены Qwen, измеренные пределы, публичный контракт и реальный MCP-проход. Массовая подготовка, полный производственный координатор, представления и выпуск остаются следующей границей DT4.
...
- Основание общего поиска реализовано: Arctic, E5 Large и Nomic возвращают по пять кандидатов, пул ёмкостью 15 позиций дедуплицируется и ранжируется Qwen3-Reranker-0.6B, а публичный JSON/MCP-ответ возвращает до шести результатов. Полный контур подготовки и выпуска ещё не закрыт.
...
- `G-EXECUTION-BASE` закрыт до начала реализации. Planning revision `aa7a7d45033098b1690691c47b4dadbd5f7b084e` отделена от исполнительных редакций; каждый gate повторён на точном кандидате, а ветвь A принята без активных замечаний.
