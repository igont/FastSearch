...
- Ближайшая граница дерева 4 материализована как семантический план `PV-1` в совместимом корпусе `FASTSEARCH-DT4-FOUNDATION-22-08-2026-PV2`: `.agents/DT-22-08-2026_19-18-Единый-агентский-поиск-FastSearch-PV-2`. Исполним только контур `A1 → A5`: точная исполнительная редакция, проверка зависимостей и Qwen, отдельное принятие измеренных пределов, публичный контракт, затем тонкий реальный MCP-проход. Массовая подготовка, полный координатор, представления и выпуск остаются дальним каркасом до принятия `GA-FOUNDATION`.
